...
class EncryptedKeyring(Encrypted, Keyring):
    ...
    # look for secret file that contains encrpted store password to bypass prompt
    # This is purely a sacrfifice in security for convenience and ease of use. Be
    # sure to hide the .secret file with file level security - chmod 600
    def _secret_file(self):
        root = os.environ.get('XDG_SECRET_HOME', None) or None
        try:
            with open(os.path.join(root, 'python_keyring/.secret'),'r') as f:
                return f.read().split()
        except Exception:
            return False
